...
_via_control_panel.prototype._add_view_manager_tools = function () {
  var prev_view = _via_util_get_svg_button('micon_navigate_prev', 'Показати попередній файл', 'show_prev');
  prev_view.addEventListener('click', this.via.vm._on_prev_view.bind(this.via.vm));
  this.c.appendChild(prev_view);

  var next_view = _via_util_get_svg_button('micon_navigate_next', 'Показати наступний файл', 'show_next');
...
_via_control_panel.prototype._add_region_shape_selector = function () {
  ...
  var extreme_rect = _via_util_get_svg_button('shape_extreme_rectangle', 'Покращений прямокутник', 'EXTREME_RECTANGLE');
